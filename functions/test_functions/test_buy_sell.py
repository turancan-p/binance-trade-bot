...
class TestBuySell():
    ...
    def buy(self, symbol):
        ...
        self.status['stop_loss'] = float(round(__price - (__price * 0.034), 4))
    ...
    def sell(self):
        ...
        if __price >= self.status['target_price']:
            __account_stats['budget'] = float(self.status['coin_amount'] * __coin_details.get_price())
        ...
        if __price <= self.status['stop_loss']:
    ...
    def write_console(self):
        ...
        if(self.status['in_position']):
            ...
            __current_coin_table.add_row([self.status["current_coin"], __coin_details.get_price()])
